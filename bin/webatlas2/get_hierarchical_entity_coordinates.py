...
def process(output_path, project_annotations_path, section_annotations_path, zarr_anndata_paths):
    ...
    for entity_type in categorical_entity_types:
        ...
        if len(all_entities) > 0:
            ...
            for zarr_dir in zarr_anndata_paths:
                ...
                img_name2entity2xy_coords_set[img_name] = {}
                ...
                for idx, entity in enumerate(hierarchical_entity_values):
                    if entity not in img_name2entity2xy_coords_set[img_name]:
                        img_name2entity2xy_coords_set[img_name][entity] = set([])
                    xy = spatial_xy[idx]
                    x = int(xy[0].astype(object))
                    y = int(xy[1].astype(object))
                    scaled_x = math.floor(x * sf_x)
                    scaled_y = math.floor(y * sf_y)
                    img_name2entity2xy_coords_set[img_name][entity].add((scaled_x, scaled_y))

            # 3. Now propagate the coordinates of the children to all their parents recursively
            for img_name in img_name2entity2xy_coords_set:
                for entity in entity2children:
                    for child in entity2children[entity]:
                        if child in img_name2entity2xy_coords_set[img_name]:
                            if entity not in img_name2entity2xy_coords_set[img_name]:
                                ...
                                img_name2entity2xy_coords_set[img_name][entity] = set([])
                            img_name2entity2xy_coords_set[img_name][entity].update(img_name2entity2xy_coords_set[img_name][child])
                ...
                missing = []
                for entity in img_name2entity2xy_coords_set[img_name]:
            ...
            # 4. Convert img_name2entity2xy_coords_set to img_name2entity2xy_coords_list (as sets are not JSON serializable)
            img_name2entity2xy_coords_list = {}
            for img_name in img_name2entity2xy_coords_set:
                if img_name not in img_name2entity2xy_coords_list:
                    img_name2entity2xy_coords_list[img_name] = {}
                for entity in img_name2entity2xy_coords_set[img_name]:
                    img_name2entity2xy_coords_list[img_name][entity] = list(img_name2entity2xy_coords_set[img_name][entity])

            # 5. Now output the coordinates into out_fpath
